<script>
   ...
   *
   * Hampir semua warga membuka situs ini dari HP Android, yang artinya akun
   * Google-nya sudah masuk di peramban. Bagi mereka masuk cukup dua ketukan:
   * tekan tombol, pilih akun. Tidak ada kata sandi baru yang harus dikarang
   * dan diingat.
   *
   * Yang lebih menentukan: email akun Google SUDAH DIPASTIKAN oleh Google.
   ...
   *
   * Email dan kata sandi TIDAK dihapus, hanya ditaruh di balik satu tombol.
   * Sebagian warga, terutama yang lebih tua, memang tidak punya akun Google
   * atau tidak tahu kata sandinya; menutup jalur itu berarti menutup situs
   * untuk mereka.
   *
   * CATATAN UNTUK YANG MENGUBAH BERKAS INI KEMUDIAN
   * Urutan ini pernah dibalik sekali, lalu dikembalikan lagi. Pemilik situs
   * memintanya secara khusus: warga harus bisa langsung ke akun Google-nya
   * tanpa mengetik email dan kata sandi. Kalau urutannya perlu diubah lagi,
  ...
  async function jalankan(e) {
    ...
    try {
      if (mode === "masuk") {
        ...
        beriTahu("Akun dibuat. Tautan pemastian sudah dikirim ke " + f.email.trim() + ".");
      } else {
...
{:else}
  ...
  {#if !bukaEmail}
    <div class="baris-tombol">
      <button class="tombol" type="button" disabled={terkunci} onclick={() => (bukaEmail = true)}>
  ...
  {:else}
    ...
    <form class="isian-borang" onsubmit={jalankan}>
      ...
      <div class="isian">
        ...
        />
        {#if mode === "masuk"}
          <span class="petunjuk">Kalau sebelumnya Anda masuk lewat Google, pakai tombol Google di atas.</span>
        {:else if mode === "daftar"}
      ...
      {#if mode === "daftar"}
        <div class="isian">
          <label for="m-ulang">Ulangi kata sandi</label>
          <input id="m-ulang" type="password" bind:value={f.ulang} required autocomplete="new-password" />
        </div>
      {/if}

      <div class="baris-tombol">
        <button class="tombol utama" type="submit" disabled={terkunci}>
          {sibuk ? "Memproses..." : mode === "masuk" ? "Masuk" : mode === "daftar" ? "Daftar" : "Kirim tautan penggantian"}
        </button>
        <button class="tombol" type="button" disabled={terkunci} onclick={() => (bukaEmail = false)}>Kembali</button>
      </div>
      ...
      {:else if mode === "lupa"}
        <p class="catatan-borang">
          ...
          Tautan hanya dikirim bila alamat itu memang terdaftar.
        </p>
      {/if}
    </form>
  {/if}
