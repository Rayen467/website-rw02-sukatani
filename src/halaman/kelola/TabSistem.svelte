<script>
  ...
  let form = $state({
    ...
    siteUrl: "https://rayen467.github.io/website-rw02-sukatani/",
    domainTarget: "",
    apiUrl: "",
    healthUrl: "",
    mediaUrl: "",
  ...
  const dok = $derived(isi.konten?.[KONTEN.SISTEM] || null);
  ...
  $effect(() => {
    if (!dimuat && dok) {
      form = {
        ...
        domainTarget: dok.domainTarget || "",
        apiUrl: dok.apiUrl || "",
        healthUrl: dok.healthUrl || "",
        mediaUrl: dok.mediaUrl || "",
        supportEmail: dok.supportEmail || "",
        catatan: dok.catatan || ""
      };
      dimuat = true;
    }
  ...
  async function simpan() {
    sibuk = true;
    try {
      await simpanKonten(KONTEN.SISTEM, form);
      await muatKonten(KONTEN.SISTEM);
      beriTahu("Pengaturan koneksi tersimpan. Tidak perlu mengubah source code untuk nilai ini.");
    } catch (err) {
      beriTahu(err?.code ? pesanRamah(err) : (err?.message || "Pengaturan belum tersimpan."));
    } finally { sibuk = false; }
  }
  ...
  async function uji() {
    ...
    if (!tujuan) {
      cek = { status: "gagal", pesan: "Isi URL health check atau API terlebih dahulu." };
      return;
    }
    ...
    try {
      ...
      cek = r.ok
        ? { status: "ok", pesan: `Terhubung · HTTP ${r.status} · ${ms} ms` }
        : { status: "gagal", pesan: `Server merespons HTTP ${r.status} · ${ms} ms` };
    } catch (err) {
      cek = { status: "gagal", pesan: "Belum bisa dijangkau dari browser. Periksa URL, HTTPS, CORS, DNS, atau status server." };
    }
  }
...
<section class="blok">
  ...
  <form class="isian-borang" onsubmit={(e) => { e.preventDefault(); simpan(); }}>
    ...
    <div class="isian"><label for="sys-domain">Domain tujuan</label><input id="sys-domain" bind:value={form.domainTarget} placeholder="rw02sukatani.id" /><span class="petunjuk">Catatan konfigurasi domain. Setelah DNS/provider diarahkan, URL website aktif di atas dapat diganti tanpa menyentuh kode.</span></div>
    ...
    <div class="isian wide"><label for="sys-note">Catatan operasional</label><textarea id="sys-note" bind:value={form.catatan} placeholder="Contoh: domain dikelola di penyedia X, masa berlaku sampai..., PIC..."></textarea></div>
...
<section class="blok">
  <div class="kepala-bagian"><h2>Catatan perubahan domain</h2></div>
  <div class="catatan"><b>Yang bisa diganti di dashboard:</b> URL referensi website, API, health check, media/CDN, email sistem, dan domain tujuan. <b>Yang tetap membutuhkan panel penyedia:</b> DNS (A/CNAME), custom domain GitHub Pages/Vercel, SSL, dan izin Firebase Authorized Domains. Itu konfigurasi infrastruktur, bukan source code.</div>
</section>

<style>
  .sys-status{margin-top:16px;padding:12px 14px;border-radius:10px;background:#f3f6f4;color:#53635d}.sys-status.sys-ok{background:#eaf8f1;color:#116b4f}.sys-status.sys-gagal{background:#fff2ef;color:#9b3a2d}.sys-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.sys-grid article{display:grid;gap:8px;padding:16px;border:1px solid #dce7e1;border-radius:12px;background:#fff}.sys-grid span{font-size:12px;color:#6b7d75}.sys-grid b{overflow-wrap:anywhere}.sys-grid .tombol{width:max-content}@media(max-width:720px){.sys-grid{grid-template-columns:1fr}}
</style>