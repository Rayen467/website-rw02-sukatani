...
<section class="blok">
  ...
  <form class="isian-borang" onsubmit={(e) => { e.preventDefault(); jalan("rt", async () => {
    const foto = (await bacaFoto(fotoRT, SISI_POTRET)) || rt.foto || "";
...
<section class="blok">
  ...
  <form class="isian-borang" onsubmit={(e) => { e.preventDefault(); jalan("majelis", async () => { await simpanKonten(KONTEN.MAJELIS_TAKLIM, mt); muatKonten(KONTEN.MAJELIS_TAKLIM); }); }}>
    ...
    <div class="catatan"><b>Dokumen pendaftaran KUA.</b> Data berikut berasal dari Surat Permohonan Pendaftaran yang ada di arsip.</div>
...
<section class="blok">
  ...
  <div class="catatan" style="margin-bottom:18px">
    Data awal diisi dari AD/ART Pengelolaan GOR Nurani dan arsip yang diberikan. Jadwal, tarif, nama pengurus, serta aturan di sini langsung dipakai mini-system GOR publik.
  </div>
  <form class="isian-borang" onsubmit={(e) => { e.preventDefault(); jalan("gor", async () => { await simpanKonten(KONTEN.GOR_NURANI, gor); muatKonten(KONTEN.GOR_NURANI); }); }}>
    ...
    <div class="catatan"><b>Kepengurusan GOR.</b> Nama yang belum ada di arsip boleh dikosongkan sampai data resmi diberikan.</div>
    <div class="isian"><label for="gor-ketua-nama">Nama Ketua GOR</label><input id="gor-ketua-nama" bind:value={gor.ketuaNama} /></div>
    <div class="isian"><label for="gor-pelaksana-nama">Nama Pelaksana Harian</label><input id="gor-pelaksana-nama" bind:value={gor.pelaksanaNama} /></div>
    <div class="isian"><label for="gor-bendahara-nama">Nama Bendahara GOR</label><input id="gor-bendahara-nama" bind:value={gor.bendaharaNama} /></div>
    ...
    <div class="catatan"><b>Layanan, jadwal, dan peminjaman.</b></div>
    ...
    <div class="isian"><label for="gor-komp-rw">Komposisi tarif RW 02 lainnya</label><textarea id="gor-komp-rw" bind:value={gor.komposisiRw02Lain}></textarea></div>
    <div class="isian"><label for="gor-tarif-luar">Tarif luar RW 02</label><input id="gor-tarif-luar" bind:value={gor.tarifLuarRw02} /></div>
    ...
    <div class="isian"><label for="gor-profit">Alokasi hasil pengelolaan</label><textarea id="gor-profit" bind:value={gor.alokasiProfit}></textarea><span class="petunjuk">Satu alokasi per baris.</span></div>
...
<section class="blok">
  ...
  <form class="isian-borang" onsubmit={(e) => { e.preventDefault(); jalan("kontak", async () => { const g = (await bacaFoto(gambarPeta, SISI_GAMBAR_PETA)) || kn.gambarPeta || ""; await simpanKonten(KONTEN.KONTAK, { ...kn, gambarPeta: g }); gambarPeta = null; muatKonten(KONTEN.KONTAK); }); }}>
